...
- Local processes while an on-demand Agent Browser session is active. The fork's `agent-browser inject` command sends Cookie commands only to the explicitly named local session through agent-browser's stdin batch protocol, but every same-user process retains the ability to drive that session. Prefer `--domain` to minimize the identity granted to each job, and close the named session when the job finishes.
