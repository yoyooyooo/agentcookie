...
- Local processes while an on-demand Agent Browser session is active. The fork's `agent-browser inject` command discovers only the selected session's loopback CDP endpoint and rejects remote CDP hosts, but every same-user process retains the ability to drive that local session. Prefer `--domain` to minimize the identity granted to each job, and close the named session when the job finishes.
